...
static int tried_embedded_load = 0;

int try_embedded_load(void* embedded_payload, u32 embedded_payload_size)
{
  if (tried_embedded_load){
    return 1;
  }
  
  tried_embedded_load = 1;
  printf("Trying to load embedded payload %x:%d... ", embedded_payload, embedded_payload_size);
  
  if (!(0x80000000 <= (u32)embedded_payload || (u32)embedded_payload <= 0x817fffff || 0x90000000 <= (u32)embedded_payload || (u32)embedded_payload <= 0x937fffff)){
    printf("Fail\n");
    return 1;
  }
  
  memcpy(code_buffer, embedded_payload, embedded_payload_size);
  printf("OK.\n");
  return 0;
}
...
int main(void* embedded_payload, u32 embedded_payload_size)
{
	dsp_reset();
 ...
 restart:
	err = try_embedded_load(embedded_payload, embedded_payload_size);
    ...
    udelay(2000); // Let me see the screen before you die
		entry();
